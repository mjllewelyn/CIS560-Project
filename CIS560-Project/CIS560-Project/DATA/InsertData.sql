...
SET @sql = 'BULK INSERT CrossCountry.[User] FROM "' + @path + 'User.csv" WITH (FIRSTROW=1, FIELDTERMINATOR='','', ROWTERMINATOR=''\n'')'
EXEC(@sql)

SET @sql = 'BULK INSERT CrossCountry.[Coach] FROM "' + @path + 'Coach.csv" WITH (FIRSTROW=1, FIELDTERMINATOR='','', ROWTERMINATOR=''\n'')'
EXEC(@sql)

SET @sql = 'BULK INSERT CrossCountry.[Team] FROM "' + @path + 'Team.csv" WITH (FIRSTROW=2, FIELDTERMINATOR='','', ROWTERMINATOR=''\n'')'
EXEC(@sql)

SET @sql = 'BULK INSERT CrossCountry.[Runner] FROM "' + @path + 'Runner.csv" WITH (FIRSTROW=1, FIELDTERMINATOR='','', ROWTERMINATOR=''\n'')'
EXEC(@sql)
